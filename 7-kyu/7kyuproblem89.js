...
function spoonerize(words) {
    let str = words.split(' ')
    let firstLetter = (str[0][0]) // first letter
    let secondLetter = (str[str.length-1][0]) // second letter
    let firstWord = secondLetter + str[0].slice(1)
    let secondWord = firstLetter + str[str.length-1].slice(1)
    str.shift()
